...
if(isset($_GET["order_id"]))
{
	...
	foreach($order_result as $order)
	{
        $flg1 = 0;
        $payment_method = "";
        if($order["Cash"] > 0 && $order["Card"] > 0 && $order["Upi"] > 0 && $flg1 != 1){
        $payment_method = "Cash + Card + UPI";
        $flg1 = 1;
        }
        elseif($order["Cash"] > 0 && $order["Card"] > 0 && $flg1 != 1){
        $payment_method = "Cash + Card";
        $flg1 = 1;
        }
        elseif($order["Cash"] > 0 && $order["Upi"] > 0 && $flg1 != 1){
        $payment_method = "Cash + UPI";
        $flg1 = 1;
        }
        elseif($order["Card"] > 0 && $order["Upi"] > 0 && $flg1 != 1){
        $payment_method = "Card + UPI";
        $flg1 = 1;
        }
        elseif($order["Cash"] > 0 && $flg1 != 1){
        $payment_method = "Cash";
        $flg1 = 1;
        }
        elseif($order["Card"] > 0 && $flg1 != 1){
        $payment_method = "Card";
        $flg1 = 1;
        }
        elseif($order["Upi"] > 0 && $flg1 != 1){
        $payment_method = "UPI";
        $flg1 = 1;
        }
		...
		<tr>
			<td>
				<table width="100%" border="0" cellpadding="5" cellspacing="5">
					<tr>
						<td width="10%"><b>Bill No:- </b>'.$order["order_number"].'</td>
						<td width="10%"><b>Table No:- </b>'.$order["order_table"].'</td>
						<td width="10%"><b>Room No:- </b>'.$order["room_number"].'</td>
						<td width="10%" ><b>Payment Mode:- </b>'.$payment_method.'</td>
						<td width="10%" ><b>Date:- </b>'.$order["order_date"].'</td>
						<td width="10%" ><b>Time:- </b>'.$order["order_time"].'</td>
					</tr>
				</table>
			</td>
		</tr>
		...
		$output .= '
						<tr>
							...
							'<br /> <br /><b>Waiter : </b>'.$order["order_waiter"].'
							</td>
