...
class rms
{
	public $base_url = 'http://127.0.0.1/source-code-restaurant-management-system-version-1/';
	public $connect;
	public $query;
	public $statement;
	public $cur;
	...
	function rms()
	{
		$this->connect = new PDO("mysql:host=127.0.0.1;dbname=rms", "root", "");
	...
	function Get_monthly_sales()
	{
		...
		$this->query = "
        SELECT Sum(order_net_amount) AS 'total' FROM order_table
        WHERE Month(order_date) = '".date("m", strtotime($month))."'
	...
	function Get_hourly_sales()
	{
		$date = date('Y-m-d');

		$this->query = "
        SELECT SUM(order_net_amount) as 'total' FROM order_table
        WHERE  order_time  BETWEEN '20:00:00' AND '23:59:00'
        AND order_date = '".$date."'
        AND  order_status = 'Completed'";
	...
	{
		$this->query = "
		SELECT SUM(Cash) AS total FROM order_table
    ...
    function Get_card_sales()
	{
		$this->query = "
		SELECT SUM(Card) AS total FROM order_table
		WHERE order_status = 'Completed'
		";
    ...
    function Get_outstanding_sales()
	{
		$this->query = "
		SELECT SUM(order_net_amount) AS total FROM order_table
